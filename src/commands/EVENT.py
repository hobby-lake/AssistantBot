...
class PointsInfoModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # ファイル準備
        event_file = BASE.get_event_path(guild_id, "event", "scheduled", event_id)
        index_file = BASE.get_event_path(guild_id, "event", "scheduled", "index")
        event_file.parent.mkdir(parents=True, exist_ok=True)
        index_file.parent.mkdir(parents=True, exist_ok=True)

        # index更新
        index = BASE.dataload(index_file)
        index[event_id] = {
            "name": self.base_info["name"],
            "開催者": author.id,
            "開始": start.isoformat()
        }
        BASE.datasave(index, index_file)
        ...
        BASE.datasave(event_data, event_file)
